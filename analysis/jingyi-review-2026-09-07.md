...
| dataset trim | cycles 1-4 built 2026-09-09 (workspace): her cuts read off sotac_raw (`analysis/trim-census.md`); detector `lib/trimDetect.ts` reproduces her recent cuts (starts within 6 frames in 97 %, ends within 3 in 92 %); trim panel in the Annotations tab; Trim page; executor `backend/trim.py` + `POST /api/trim` (verified against her own cuts on the mirror). Not yet a PR; her three questions open |
