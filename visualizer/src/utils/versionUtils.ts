...
// Optional sub-directory inside the repo that acts as the dataset root.
// Some tactile datasets (e.g. summer-dong/lerobot-ball-pickplace-0813) store
// one self-contained v3-shaped dataset PER EPISODE FOLDER
// (`ZQFZ-01/<timestamp>/meta/info.json`, `data/`, `videos/`, ...). Setting a
// prefix (via the `?root=` search param, see episode-viewer) points every
// dataset URL at that folder without touching any call sites.
let datasetPathPrefix = "";

export function setDatasetPathPrefix(prefix: string | null | undefined): void {
  let p = (prefix ?? "").trim();
  p = p.replace(/^\/+|\/+$/g, "");
  datasetPathPrefix = p ? p + "/" : "";
}

export function getDatasetPathPrefix(): string {
...
export async function getDatasetInfo(repoId: string): Promise<DatasetInfo> {
  ...
  const cacheKey = `${repoId}::${datasetPathPrefix}`;
  const cached = datasetInfoCache.get(cacheKey);
  if (cached && now < cached.expiry) {
    // Keep insertion order fresh so the cache behaves closer to LRU.
    datasetInfoCache.delete(cacheKey);
    datasetInfoCache.set(cacheKey, cached);
...
export async function getDatasetStats(
  ...
  const now = Date.now();
  const statsCacheKey = `${repoId}::${datasetPathPrefix}`;
  const cached = datasetStatsCache.get(statsCacheKey);
  ...
  datasetStatsCache.set(statsCacheKey, {
    data,
    expiry: Date.now() + CACHE_TTL_MS,
  });
  return data;
