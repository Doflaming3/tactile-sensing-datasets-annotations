...
function HomeInner() {
  ...
  useEffect(() => {
    try {
      setRecent(JSON.parse(localStorage.getItem("recent_datasets") ?? "[]"));
    } catch {}
  }, []);
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden">
      ...
      <div className="fixed inset-0 -z-0 bg-[radial-gradient(ellipse_at_top,rgba(34,211,238,0.08)_0%,rgba(0,0,0,0)_55%)]" />
      ...
      <div className="relative z-10 h-screen flex flex-col items-center justify-center text-white text-center animate-fade-in-up px-4">
        ...
        {/* Subtitle */}
        <p className="text-white/55 text-base md:text-lg mb-8 max-w-lg">
          Visuo-tactile robot datasets, video, per-taxel forces, RGBD, and
          annotations in one view
        </p>
        ...
        {/* Feature line */}
        <p className="mt-10 max-w-xl text-[13px] leading-6 text-white/35">
          {FEATURES.map((f, i) => (
            <span key={f}>
        ...
        <p className="mt-8 text-[11px] text-white/30">
          Built on the LeRobot dataset visualizer ·{" "}
          <a
            href="https://github.com/Jingyi-Z/lerobotac"
            target="_blank"
