...
export type EpisodeData = {
  ...
  /**
   * Full-rate tactile/sensor arrays for `observation.sensors.*` features
   * whose shape has 2+ dims (e.g. a Paxini fingertip `[2, 52, 3]` or a
   * buffered `[10, 52, 3]`). These are excluded from the scalar charts, so
   * without this field they would be dropped entirely. Keyed by feature
   * name; frames are NOT downsampled (the tactile panel needs every frame).
   */
  sensorFrames?: SensorFramesMap;
};

export type SensorTaxelFrames = {
  /** Feature shape from meta/info.json, e.g. [2, 52, 3]. */
  shape: number[];
  /** One nested array per source frame, matching `shape`. */
  frames: unknown[];
  /** Per-frame timestamps in seconds, parallel to `frames`. */
  timestamps: number[];
};
export type SensorFramesMap = Record<string, SensorTaxelFrames>;
...
async function loadEpisodeDataV3(
  ...
  try {
      ...
      new Set([
        ...
        ...Object.entries(info.features)
          .filter(([key, feature]) => {
            ...
            // Scalars/vectors feed the charts. Multi-dim
            // observation.sensors.* features (tactile taxel arrays) are
            // read too, for the tactile panel — they are small (KBs/frame)
            // unlike image features, which stay excluded.
            const isSensorArray =
    ...
    // Only request columns that exist in this file — optional columns
    // (language_instruction*, language_persistent/events) are present in
    // company-format data but absent from plain recordings, and hyparquet
    // hard-fails on missing columns.
    try {
      const available = new Set(await parquetColumnNames(parquetFile));
      for (let i = v3DataColumns.length - 1; i >= 0; i--) {
        if (!available.has(v3DataColumns[i])) v3DataColumns.splice(i, 1);
      }
    } catch {
      // schema probe failed — fall through with the full list
    }
    ...
    // Full-rate tactile arrays for the tactile panel (never downsampled —
    // contact transients live between chart sample points).
    const sensorFrames: SensorFramesMap = {};
    for (const [key, feature] of Object.entries(info.features)) {
      if (!key.startsWith("observation.sensors.")) continue;
      if (feature.shape.length < 2) continue;
      ...
      for (const row of episodeRows) {
        ...
        timestamps.push(
          typeof t === "number" ? t : typeof t === "bigint" ? Number(t) : 0,
        );
      }
      if (frames.length > 0) {
        sensorFrames[key] = { shape: feature.shape, frames, timestamps };
      }
    }
...
function processEpisodeDataForCharts(
  ...
  // Real per-row timestamps from the parquet, made episode-relative. The old
  // code synthesized a fake axis from the video container duration, which
  // carries muxing drift (e.g. 30s axis for a 21s episode on merged
  // datasets) and skewed every chart x-axis and downstream consumer.
  const rowTs = (r: unknown): number | null => {
    const v = (r as Record<string, unknown>)?.["timestamp"];
    if (typeof v === "number" && !isNaN(v)) return v;
    if (typeof v === "bigint") return Number(v);
    return null;
  };
  const firstTs = episodeData.length > 0 ? rowTs(episodeData[0]) : null;

  const chartData = episodeData.map((row, index) => {
    const obj: Record<string, number> = {};

    const realTs = rowTs(row);
    if (realTs !== null && firstTs !== null) {
      obj["timestamp"] = realTs - firstTs;
    } else {
      // Fallback (no timestamp column): synthesize from video duration.
      let videoDuration = episodeData.length;
      if (episodeMetadata) {
        videoDuration =
