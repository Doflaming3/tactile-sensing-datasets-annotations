...
type VideoPlayerProps = {
  ...
  onVideosReady?: () => void;
  /** Fill the parent grid tile's height (dashboard layout). */
  fill?: boolean;
};
...
export const SimpleVideosPlayer = ({
  ...
  return (
    <>
      ...
      {/* Videos: auto-fit grid — always fills the row edge-to-edge, so the
          camera row lines up exactly with the sensor panel below at every
          viewport width, monitor ratio, and zoom level. */}
      <div
        className={`grid gap-x-2 gap-y-6 grid-cols-[repeat(auto-fit,minmax(min(100%,340px),1fr))] ${
          fill ? "h-full min-h-0 items-stretch" : ""
        }`}
      >
                ...
                <video
                  ...
                  className={`w-full object-contain ${
                    isEnlarged
                      ...
                      : fill
                        ? "h-full max-h-full"
                        : // viewport-height budget: videos row stays <=36vh so
                          // videos + sensors + charts fit one screen unscrolled
                          "max-h-[36vh]"
