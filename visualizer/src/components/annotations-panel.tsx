...
export const AnnotationsPanel: React.FC<Props> = ({ cameraKeys }) => {
  ...
  const handleSaveDataset = async () => {
    // Backend configured (local dev): full parquet export.
    ...
    // No backend (deployed Space): commit annotations JSON to the dataset
    // repo on the Hub as the signed-in user. The Hub enforces write access.
    if (!ident.repoId || episodeId == null) {
      setExportStatus("No dataset repo in context.");
      return;
    }
      ...
      <div className="annotation-actionbar">
        ...
        <div className="actionbar-actions">
          ...
          <button
            ...
            title={
              backendEnabled
                ? "Export all annotations via the local backend"
                : "Commit this episode's annotations to the dataset repo on the Hub (needs sign-in with write access)"
            }
      ...
      <section className="annotation-composer">
        <div className="composer-copy">
          <span
            ...
            title="Adds task phrasing, subtask, plan, memory, speech, or non-spatial VQA atoms. Task phrasings are saved at episode start."
          >
            Add text annotation
          </span>
        </div>
