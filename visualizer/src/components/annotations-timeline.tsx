...
const TRACK_GROUPS = [
  ...
  },
  {
    ...
    tracks: [
      ...
      // Auto-generated tactile events ([auto:…] interjections) get their own
...
export const AnnotationsTimeline: React.FC<Props> = ({ duration }) => {
  ...
  // Seek + select helper. Clicking a marker jumps the playhead AND starts
  // playback — reviewing an event means watching it, not staring at a
  // paused frame. (Boundary drags still pause via their own handlers.)
  ...
  const lanes = useMemo(() => {
    ...
    atoms.forEach((a, i) => {
      if (a.style === "memory") {
        ...
        const m = a.content.match(/^\[auto:(\w+)\]\s*(.*)$/);
        tactile.push({
          kind: "tick",
          t: a.timestamp,
          label: m ? m[2] : a.content,
          atom: a,
          atomIdx: i,
          subtype: m ? m[1] : undefined, // confidence
        });
                    ...
                    >
                      ...
                      {tk.render === "tick" &&
                        (
                          ...
                          // Clamp: atoms can carry timestamps beyond the
                          // episode duration (e.g. from over-long raw
                          // streams); never render outside the track.
