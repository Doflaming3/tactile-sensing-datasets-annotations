...
type DataGraphProps = {
  data: ChartRow[][];
  onChartsReady?: () => void;
  /** Start in the merged single-chart view. */
  defaultCombined?: boolean;
  /** Fill the parent's height (dashboard grid tile) instead of aspect caps. */
  fill?: boolean;
};
...
export const DataRecharts = React.memo(
  ({ data, onChartsReady, defaultCombined = false, fill = false }: DataGraphProps) => {
    ...
    return (
      <div className={fill ? "h-full min-h-0 flex flex-col" : undefined}>
        {data.length > 1 && (
        ...
        {expanded ? (
          ...
          <div
            className={`grid min-[900px]:grid-cols-2 grid-cols-1 gap-4 ${
              fill ? "flex-1 min-h-0" : ""
            }`}
          >
            {data.map((group, idx) => (
...
const SingleDataGraph = React.memo(
  ({
    ...
    tall?: boolean;
    fill?: boolean;
    /** With fill: put the legend in a scrollable strip below the plot
        (split view) instead of a right-hand column (combined view). */
    legendBelow?: boolean;
  }) => {
    ...
    return (
      <div className={`w-full bg-[var(--surface-1)]/40 rounded-lg border border-white/10/50 p-3 ${fill ? (legendBelow ? "h-full min-h-0 flex flex-col" : "h-full min-h-0 flex flex-row items-stretch") : ""}`}>
        ...
        <div
          className={`${
            fill
              ...
              : tall
                ? "w-full aspect-[16/7] min-h-[280px] max-h-[40vh]"
                : "w-full aspect-[16/9] min-h-[180px] max-h-[28vh]"
          }`}
        ...
        {fill ? (
          legendBelow ? (
            <div className="shrink-0 max-h-[42%] overflow-y-auto mt-1">
              <CustomLegend />
            </div>
          ) : (
            <div className="w-[300px] shrink-0 min-h-0 overflow-y-auto pl-2">
              <CustomLegend />
            </div>
          )
        ) : (
