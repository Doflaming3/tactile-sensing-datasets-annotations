...
async function main() {
  ...
  for (const m of metas) {
    ...
    if (existsSync(dir)) {
      ...
      if (files.length) {
        const raw = buildSeriesFromRawCsvs(files.map((f) => readFileSync(join(dir, f), "utf-8")), layout, undefined, { profile: PROFILE });
        if (raw) series = clipSeries(raw, timestamps[timestamps.length - 1] + 0.1);
      }
    }
    if (!series) series = buildSeriesFromSensorFrames(frames, timestamps, layout, undefined, PROFILE);
    if (!series || !gripper) continue;

    const result = detectEvents(series, gripper, {}, arm, { profile: PROFILE });
